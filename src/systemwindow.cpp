    ...
    m_system(system)
{
    ...
    this->setWindowTitle(
                QString(tr("System %1"))
                .arg(system));
    ui->twDetails->header()->setSectionResizeMode(QHeaderView::ResizeToContents);
    ui->twDetails->sortItems(0, Qt::SortOrder::AscendingOrder);
...
SystemWindow::~SystemWindow()
{
    otpConsumer->removeConsumerSystem(m_system);
    delete ui;
}
...
QTreeWidgetItem* SystemWindow::getGroupItem(group_t group)
{
    ...
    if (groupItems.isEmpty()) return Q_NULLPTR;
...
QTreeWidgetItem* SystemWindow::getPointItem(ACN::OTP::group_t group, ACN::OTP::point_t point)
{
    ...
    if (!groupItem) return Q_NULLPTR;

    QTreeWidgetItem* pointItem = Q_NULLPTR;
...
QTreeWidgetItem* SystemWindow::getPositionItem(group_t group, point_t point)
{
    ...
    if (!pointItem) return Q_NULLPTR;

    QTreeWidgetItem* positionItem = Q_NULLPTR;
...
QTreeWidgetItem* SystemWindow::getPositionDetailItem(group_t group, point_t point, PointsPosRotDetailHeaders_e detailType)
{
    ...
    if (!positionItem) return Q_NULLPTR;

    QTreeWidgetItem* valueItem = Q_NULLPTR;
...
QTreeWidgetItem* SystemWindow::getRotationItem(group_t group, point_t point)
{
    ...
    QTreeWidgetItem* rotationItem = Q_NULLPTR;
    for (int idx = 0; idx < pointItem->childCount(); idx++)
    {
...
QTreeWidgetItem* SystemWindow::getDetailItem(group_t group, point_t point)
{
    ...
    QTreeWidgetItem* detailItem = Q_NULLPTR;
    for (int idx = 0; idx < pointItem->childCount(); idx++)
    {
